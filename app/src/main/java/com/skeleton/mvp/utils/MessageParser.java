...
/*
     Create clickable spans within a TextView with pattern matching!
...
public class MessageParser {
    ...
    public MessageParser addPattern(Pattern pattern, SpannableStyleListener spanStyles, SpannableClickedListener listener) {
        patterns.add(new SpannablePatternItem(pattern, spanStyles, listener));
        return this;
    }

    public MessageParser addPattern(Pattern pattern, SpannableStyleListener spanStyles) {
        addPattern(pattern, spanStyles, null);
        return this;
    }

    public MessageParser addPattern(Pattern pattern) {
        addPattern(pattern, null, null);
        return this;
    }

    public MessageParser addPattern(Pattern pattern, int textColor) {
        addPattern(pattern, textColor, null);
        return this;
    }

    public MessageParser addPattern(Pattern pattern, int textColor, SpannableClickedListener listener) {
    ...
    public MessageParser addPattern(Pattern pattern, SpannableClickedListener listener) {
    ...
    public static String parseMentions(String message) {
        final String regex = "<a(.*?)mention:\\/\\/(\\-?\\d*)\"(.*?)>(.*?)<\\/a>";
        final Pattern pattern = Pattern.compile(regex);
        final Matcher matcher = pattern.matcher(message);
        String finalMessage = message;
        while (matcher.find()) {
//            // ID comes in group 2.
//            // Name comes in group 4.
            finalMessage = finalMessage.replace(matcher.group(0), "[fuguUser id=" + matcher.group(2) + "]" + matcher.group(4) + "[/fuguUser]");
        }
        return finalMessage;
    }
