...
//                    }
    ...
    private void apiGetInvitedMember() {
        ...
        com.skeleton.mvp.data.network.CommonParams.Builder commonParams = new com.skeleton.mvp.data.network.CommonParams.Builder();
        ...
        commonParams.add("user_type", "PENDING");
        commonParams.add("page_start", pageStart);

        RestClient.getApiInterface(true).getAllMembers(fcCommonResponse.getData().getUserInfo().getAccessToken(), BuildConfig.VERSION_CODE, ANDROID, commonParams.build().getMap())
                .enqueue(new com.skeleton.mvp.data.network.ResponseResolver<AllMemberResponse>() {
                    ...
                    public void onSuccess(AllMemberResponse allMemberResponse) {
                        ...
                        for (int i = 0; i < allMemberResponse.getData().getPendingMemberList().size(); i++) {
                            PendingMember invitedMembers = allMemberResponse.getData().getPendingMemberList().get(i);
                            if (invitedMembers.getStatus().equalsIgnoreCase("NOT_EXPIRED") || invitedMembers.getStatus().equalsIgnoreCase("RE_INVITED")) {
                                if (!TextUtils.isEmpty(allMemberResponse.getData().getPendingMemberList().get(i).getEmail())) {
                                    alreadyInvitedList.add(new InvitedMembers(invitedMembers.getEmail(),
                                            true, invitedMembers.getDateTime(), invitedMembers.getStatus(),
                                            false));
                                } else {
                                    alreadyInvitedList.add(new InvitedMembers(invitedMembers.getContactNumber(),
                                            false, invitedMembers.getDateTime(), invitedMembers.getStatus(),
                                            false));
                                }
                            }
                        }
                        if (alreadyInvitedList.size() == 0) {
                            ...
                            tvMessage.setText(R.string.no_pending_invites);
                        } else {
