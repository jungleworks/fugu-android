...
class JitsiCallActivity : com.skeleton.mvp.ui.base.BaseActivity() {
                ...
                try {
                    ...
                    if (data.getString(VIDEO_CALL_TYPE) == FuguAppConstant.JitsiCallType.READY_TO_CONNECT_CONFERENCE.toString()
                        ...
                        if (!OngoingCallService.NotificationServiceState.isConferenceServiceRunning) {
                            ...
                            val answerIntent = Intent(this@JitsiCallActivity, VideoConfActivity::class.java)
                        ...
                        mediaPlayer = if (videoCallModel?.callType.equals("VIDEO")) {
                            MediaPlayer.create(this@JitsiCallActivity, R.raw.busy_tone)
                        } else {
                            MediaPlayer.create(this@JitsiCallActivity, R.raw.busy_tone, aa, 1)
                        }
                        ...
                        runOnUiThread {
                            ...
                            if (mediaPlayer != null) {
                                mediaPlayer?.stop()
                                mediaPlayer = MediaPlayer.create(this@JitsiCallActivity, R.raw.busy_tone)
                                mediaPlayer?.isLooping = false
                                mediaPlayer?.start()
                            }
                        }
                ...
                runOnUiThread {
                    ...
                    try {
                        ...
                        if (fayeVideoCallResponse.statusCode == 415) {
                            ...
                            Toast.makeText(this@JitsiCallActivity, "$name does not have the updated app, Calling using old SDK... ", Toast.LENGTH_LONG).show()
    ...
    private fun initCall(link: ArrayList<String>) {
    ...
    private fun startCall(isSilent: Boolean, linkArray: ArrayList<String>) {
        val startCallJson = JSONObject()
    ...
    private fun startCallIOS(isSilent: Boolean, linkArray: ArrayList<String>) {
        val startCallJson = JSONObject()
        startCallJson.put(IS_SILENT, isSilent)
