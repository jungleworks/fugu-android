...
class MentionInsertionLogic {
    ...
    void insertMention(final Mentionable mention) {
        ...
        if (start != -1) {
            final int newCursorPosition = start + Html.fromHtml(mention.getMentionName().toString()).length();
            editText.getText().delete(start, cursorPosition);
            editText.getText().insert(start, Html.fromHtml(mention.getMentionName().toString()));
